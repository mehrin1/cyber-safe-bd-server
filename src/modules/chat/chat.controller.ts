...
export const chatController = {
  ...
  async sendMessage(req: Request, res: Response) {
    try {
      ...
      if (error instanceof Error && error.message === "CHAT_PROVIDER_ERROR:404") return res.status(503).json({ success: false, message: "SafeBot's Gemini model was not found. Set GEMINI_MODEL to gemini-2.5-flash-lite." });
